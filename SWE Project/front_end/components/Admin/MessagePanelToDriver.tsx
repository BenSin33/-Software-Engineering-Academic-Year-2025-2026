// components/Admin/MessagePanelToDriver.tsx
...
interface MessagePanelProps {
  adminId: number;       // Admin UserID (người gửi = 1)
  driverUserId: number;  // Driver UserID (người nhận)
  driverName: string;    // Tên driver
  onClose: () => void;
}

const MessagePanelToDriver: React.FC<MessagePanelProps> = ({
  adminId,
  driverUserId,
  driverName,
  onClose,
  ...
  const loadMessages = async () => {
    try {
      ...
      console.log('📥 Admin loading messages with Driver:', { adminId, driverUserId });
      
      const data = await fetchMessages(adminId, driverUserId);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('📤 Admin sending message to Driver:', messageData);
        ...
        {loading && messages.length === 0 ? (
          ...
          <>
            {messages.map((message, index) => {
              // 🔧 Admin gửi (senderId = adminId) -> hiển thị BÊN PHẢI
              // Driver gửi (senderId = driverUserId) -> hiển thị BÊN TRÁI
                ...
                <React.Fragment key={message.id}>
                  ...
                  >
                    <div style={{
                      ...
                      borderRadius: isFromAdmin ? '16px 16px 4px 16px' : '16px 16px 16px 4px',
                      backgroundColor: isFromAdmin ? '#FFAC50' : 'white',
...
export default MessagePanelToDriver;